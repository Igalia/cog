...
keyboard_on_keymap(void *data, struct wl_keyboard *wl_keyboard, uint32_t format, int32_t fd, uint32_t size)
{
    ...
    xkb_data.keymap = xkb_keymap_new_from_string(xkb_data.context,
                                                 (char *) mapping,
    ...
    xkb_data.indexes.control = xkb_keymap_mod_get_index(xkb_data.keymap, XKB_MOD_NAME_CTRL);
    xkb_data.indexes.alt = xkb_keymap_mod_get_index(xkb_data.keymap, XKB_MOD_NAME_ALT);
    xkb_data.indexes.shift = xkb_keymap_mod_get_index(xkb_data.keymap, XKB_MOD_NAME_SHIFT);
}
...
handle_key_event(CogWlPlatform *self, uint32_t key, uint32_t state, uint32_t time)
{
    ...
    if (xkb_data.compose_state != NULL && state == WL_KEYBOARD_KEY_STATE_PRESSED &&
        xkb_compose_state_feed(xkb_data.compose_state, keysym) == XKB_COMPOSE_FEED_ACCEPTED &&
                ...
                uint32_t            state)
{
    ...
    if (state == WL_KEYBOARD_KEY_STATE_RELEASED && wl_data.keyboard.repeat_data.key == key) {
        ...
        memset(&wl_data.keyboard.repeat_data, 0x00, sizeof(wl_data.keyboard.repeat_data));
...
static const struct wl_callback_listener frame_listener = {.done = on_surface_frame};

static void
on_presentation_feedback_sync_output(void                            *data,
...
static void
on_presentation_feedback_presented(void                            *data,
                                   struct wp_presentation_feedback *presentation_feedback,
                                   uint32_t                         tv_sec_hi,
                                   uint32_t                         tv_sec_lo,
                                   uint32_t                         tv_nsec,
...
request_frame(void)
{
    ...
    if (wl_data.presentation != NULL) {
        struct wp_presentation_feedback *presentation_feedback =
            wp_presentation_feedback(wl_data.presentation, win_data.wl_surface);
...
on_export_wl_egl_image(void *data, struct wpe_fdo_egl_exported_image *image)
{
    ...
    wpe_view_data.buffer = s_eglCreateWaylandBufferFromImageWL(
        egl_data.display, wpe_fdo_egl_exported_image_get_egl_image(wpe_view_data.image));
...
static void shm_buffer_destroy_notify(struct wl_listener *listener, void *data);
...
#    endif
    ...
    if (!surf->wl_subsurface) {
        surf->wl_subsurface =
            wl_subcompositor_get_subsurface(wl_data.subcompositor, surf->wl_surface, win_data.wl_surface);
...
init_wayland(GError **error)
{
    ...
    g_assert(wl_data.xdg_shell != NULL || wl_data.shell != NULL || wl_data.fshell != NULL);
...
create_window(GError **error)
{
    ...
    if (wl_data.xdg_shell != NULL) {
        win_data.xdg_surface = xdg_wm_base_get_xdg_surface(wl_data.xdg_shell, win_data.wl_surface);
        ...
        xdg_toplevel_add_listener(win_data.xdg_toplevel, &xdg_toplevel_listener, NULL);
...
create_popup(WebKitOptionMenu *option_menu)
{
    ...
    if (wl_data.xdg_shell != NULL) {
        ...
        popup_data.xdg_surface = xdg_wm_base_get_xdg_surface(wl_data.xdg_shell, popup_data.wl_surface);
        ...
        wl_shell_surface_set_popup(popup_data.shell_surface, wl_data.seat, wl_data.event_serial, win_data.wl_surface, 0,
                                   (win_data.height - popup_data.height), 0);
