...
{
    g_printerr ("%s: window=%p, launcher=%p\n", __func__, window, application);
...
dy_gtk_create_window (DyLauncher *launcher)
{
    ...
    GtkWidget *window = gtk_window_new (GTK_WINDOW_TOPLEVEL);  // Floating.
    gtk_window_set_titlebar (GTK_WINDOW (window), header);  // Takes ownwership of header.
    gtk_widget_set_size_request (window, 400, 200);
    gtk_container_add (GTK_CONTAINER (window), web_view);
    g_signal_connect (window, "destroy", G_CALLBACK (on_window_destroy), launcher);
    g_application_hold (G_APPLICATION (launcher));
